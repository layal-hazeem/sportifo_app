...
class _ActivePlayScreenState extends State<ActivePlayScreen> {
  ...
  void _updateDefaultRepsInput(ActiveWorkoutInProgress state) {
    ...
    if (exercise.reps != null && exercise.reps!.isNotEmpty) {
      List<String> repsList = exercise.reps!
          .split(RegExp(r'[:,/\-]+'))
          .map((e) => e.trim())
          .where((e) => e.isNotEmpty)
          .toList();
      if (repsList.isNotEmpty) {
        String target = currentSetIdx < repsList.length
            ? repsList[currentSetIdx]
            : repsList.last;
  ...
  void _onLogSet(ActiveWorkoutInProgress state, AppLocalizations l10n) {
    ...
    context.read<ActiveWorkoutCubit>().logSet(
      weight: _weightController.text,
      reps: _repsController.text,
    );
    _handlePostSetActions(
      state,
  ...
  void _handlePostSetActions(
    ...
    if (isLastSetOfExercise) {
      ...
      if (updatedState is ActiveWorkoutInProgress)
        finalSets = updatedState.completedSets;

      context.read<ActiveWorkoutCubit>().syncExerciseToServer(
        planId: widget.planId,
        exercise: exercise,
      );
      String totalTimeStr =
          '${(_globalSeconds ~/ 60).toString().padLeft(2, '0')}:${(_globalSeconds % 60).toString().padLeft(2, '0')}';
  ...
  void _showBlurryRestSheet({
    required bool isLastSet,
    required ExerciseModel? nextExercise,
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<ActiveWorkoutCubit, ActiveWorkoutState>(
      ...
      builder: (context, state) {
        ...
        String repsDisplay = exercise.reps != null
            ? exercise.reps!.replaceAll(RegExp(r'[:,/\-]+'), ' ')
            : '10 10 10';
